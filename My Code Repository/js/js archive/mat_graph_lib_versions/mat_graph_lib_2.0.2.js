var draw = {
 ...
 resizeCanvas : function() {
  var canvas = document.getElementById("canvas");
  var left_col = document.getElementById("l_col");
  console.log(left_col.clientWidth);
  var right_col = document.getElementById("r_col");
  if (null==canvas || !canvas.getContext) return;
  var axes={}, ctx = canvas.getContext("2d");
  var c_width = (document.getElementsByTagName("body")[0].clientWidth * 0.89) + (window.innerWidth*0.06) - left_col.clientWidth;
  if(c_width < 32) {
   canvas.height = 1;
   canvas.width = 1;
  } else {
   canvas.width = c_width;
   canvas.height = (document.getElementById("split_handle").clientHeight);
  };
  //redraw();
 }
}
